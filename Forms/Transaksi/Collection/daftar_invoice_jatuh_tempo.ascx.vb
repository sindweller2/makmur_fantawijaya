...
Partial Class Forms_Transaksi_Collection_daftar_invoice_jatuh_tempo
    Inherits System.Web.UI.UserControl

    Public Property per_customer() As String
        Get
            Dim o As Object = ViewState("per_customer")
            If Not o Is Nothing Then Return CStr(o) Else Return Nothing
        End Get
        Set(ByVal value As String)
            ViewState("per_customer") = value
        End Set
    End Property
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = " select nama_customer, nama_sales, id_customer,"
            sqlcom = sqlcom + " sum(isnull(nilai_idr,0) + isnull(nilai_ppn_usd,0)) as nilai_idr, sum(isnull(nilai_usd,0))  as nilai_usd"
            sqlcom = sqlcom + " from temp_piutang_customer"
            ...
            sqlcom = sqlcom + " group by nama_customer, nama_sales, id_customer"
    ...
    Sub hitung_piutang()
        Try
            ...
            sqlcom = "insert into temp_piutang_customer(id_sales, id_customer, no_so_text, nama_customer, nama_sales,"
            sqlcom = sqlcom + " tgl_invoice, tgl_jatuh_tempo, nilai_idr, nilai_usd, nilai_ppn_usd)"
    ...
    Sub print()
        Try
            ...
            If Me.per_customer = "N" Then
                reportPath = Server.MapPath("reports\lap_invoice_jatuh_tempo.rpt")
            Else
                reportPath = Server.MapPath("reports\lap_invoice_jatuh_tempo_per_customer.rpt")
            End If
            ...
            If Me.per_customer = "Y" Then
                oRD.SetParameterValue("id_customer", Me.vid_customer)
            End If
            ...
            oRD.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Server.MapPath("Pdf_files/lap_invoice_jatuh_tempo.pdf"))
            Dim vscript As String = ""
            vscript = "<script>" & vbCrLf
            vscript = vscript + "window.open('Pdf_files/lap_invoice_jatuh_tempo.pdf', '_blank', 'resizable=yes,scrollbars=yes');" & vbCrLf
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkPrint" Then
            Me.per_customer = "Y"
            Me.vid_customer = CType(e.Item.FindControl("lbl_id_customer"), Label).Text
            Me.print()
        End If
    End Sub
