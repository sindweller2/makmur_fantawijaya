...
Partial Class Forms_Transaksi_Collection_daftar_penerimaan_uang
    ...
    Sub loadgrid()
        Try
            ...
            Using con
                con.Open()
                da.Fill(ds, "penerimaan_uang")
                Me.dg_data.DataSource = ds.Tables("penerimaan_uang").DefaultView

                If ds.Tables("penerimaan_uang").Rows.Count > 0 Then
                    If ds.Tables("penerimaan_uang").Rows.Count > 10 Then
    ...
    'dendi
    Sub loadgrid_jenis_penerimaan_uang()
        Try
            ...
            sqlcom = sqlcom + " from penerimaan_uang"
            sqlcom = sqlcom + " where id_transaction_period = " & Me.dd_bulan.SelectedValue
            'sqlcom = sqlcom + " and jenis_penerimaan_uang = '" & Me.ddl_jenis_penerimaan_uang.SelectedItem.Text & "' order by id"
            sqlcom = sqlcom + " order by id"
    ...
    Protected Sub btn_new_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_new.Click
        Response.Redirect("~/detil_transaksi_penerimaan_uang.aspx?vtahun=" & Me.tb_tahun.Text & "&vbulan=" & Me.bulan & "&vid_periode=" & Me.dd_bulan.SelectedValue)
    End Sub
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Then
            Response.Redirect("~/detil_transaksi_penerimaan_uang.aspx?vid_transaksi=" & CType(e.Item.FindControl("lbl_id"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&vid_periode=" & Me.dd_bulan.SelectedValue)
        End If
    End Sub
