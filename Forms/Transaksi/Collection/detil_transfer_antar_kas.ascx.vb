...
Partial Class Forms_Transaksi_Keuangan_detil_transfer_antar_kas
    ...
    Sub GL()
        Try
            ...
            Dim keterangan As String = "Transfer antar kas/bank no. " & Me.lbl_no_transaksi.Text
            ...
            sqlcom = sqlcom + "',0," & nilai_awal & "," & Me.vmax_history_kas & ")"
            ...
            sqlcom = sqlcom + keterangan
            sqlcom = sqlcom + "'," & nilai_akhir & ",0," & Me.vmax_history_kas & ")"
            connection.koneksi.InsertRecord(sqlcom)
        ...
        Try
            ...
            If reader.HasRows Then
                ...
                If reader.Item("id_currency").ToString = "IDR" Then
                    Me.tb_kurs_awal.Text = 1
                Else
                    If String.IsNullOrEmpty(Me.tb_kurs_awal.Text) Or Me.tb_kurs_awal.Text = 1 Then
                        Me.tb_kurs_awal.Text = ""
                    End If
                End If
            End If
            reader.Close()
    ...
    Sub bindmatauangtujuan()
        Try
            ...
            If reader.HasRows Then
                ...
                If reader.Item("id_currency").ToString = "IDR" Then
                    Me.tb_kurs_akhir.Text = 1
                Else
                    If String.IsNullOrEmpty(Me.tb_kurs_akhir.Text) Or Me.tb_kurs_akhir.Text = 1 Then
                        Me.tb_kurs_akhir.Text = ""
                    End If
                End If
            End If
            reader.Close()
    ...
    Sub loaddata()
        ...
        sqlcom = sqlcom + " convert(char, tanggal_masuk, 103) as tanggal_masuk"
    ...
    Protected Sub btn_close_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_close.Click
        Response.Redirect("~/daftar_transfer_antar_kas.aspx?vtahun=" & Me.vtahun & "&vbulan=" & Me.vbulan & "&voption=" & Me.voption & "&vsearch=" & Me.vsearch & "&vpaging=" & Me.vpaging)
    End Sub
        ...
        Try
            ...
            Else
                ...
                If Me.id_transaksi = 0 Then
                    ...
                    sqlcom = sqlcom + " tanggal_masuk, nilai_biaya_provisi)"
                    ...
                    sqlcom = sqlcom + ",'" & vtgl_masuk & "'," & Decimal.ToDouble(tb_nilai_biaya_provisi.Text) & ")"
                    connection.koneksi.InsertRecord(sqlcom)
                ...
                Else
                    ...
                    sqlcom = sqlcom + " where seq = " & Me.id_transaksi
                    connection.koneksi.UpdateRecord(sqlcom)
                    tradingClass.Alert("Data sudah diupadate", Me.Page)
                End If
                Me.loaddata()
    ...
    Sub jurnal()
        ...
        sqlcom = sqlcom + "'" & vakun_kas_asal & "'," & Decimal.ToDouble(vnilai_masuk) * Decimal.ToDouble(Me.tb_kurs_awal.Text) & ",0, 'Transfer antar kas/bank no. " & Me.lbl_no_transaksi.Text & " (" & Me.tb_keterangan.Text & ")'"
    ...
    Protected Sub btn_submit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_submit.Click
        Try
            ...
            If nilai_awal = nilai_akhir Then
                ...
                reader.Read()
                If reader.HasRows Then
                    Me.vakun_kas_asal = reader.Item("account_code").ToString
                ...
                reader.Read()
                If reader.HasRows Then
                    Me.vakun_kas_tujuan = reader.Item("account_code").ToString
                End If
                reader.Close()
                connection.koneksi.CloseKoneksi()
