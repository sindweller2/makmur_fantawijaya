...
Partial Class Forms_Transaksi_Pembelian_received_expedition_invoice
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = sqlcom + " seq_penugasan_ekspedisi, received_import_expedition_invoice.is_submit,"
            sqlcom = sqlcom + " case"
            sqlcom = sqlcom + " when received_import_expedition_invoice.is_submit = 'B' then 'Belum disubmit'"
            sqlcom = sqlcom + " when received_import_expedition_invoice.is_submit = 'S' then 'Sudah disubmit'"
            ...
            Else
                If Me.dd_submit.SelectedValue = "B" Then
                    sqlcom = sqlcom + " and received_import_expedition_invoice.is_submit = 'B'"
                Else
                    sqlcom = sqlcom + " and received_import_expedition_invoice.is_submit = 'S'"
                End If
            End If

            sqlcom = sqlcom + " order by received_import_expedition_invoice.invoice_no"
    ...
    Protected Sub btn_new_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_new.Click
        Response.Redirect("~/detil_received_expedition_invoice.aspx?vtahun=" & Me.tb_tahun.Text & "&vbulan=" & Me.bulan)
    End Sub
        ...
        Try
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    sqlcom = Nothing
                    sqlcom = "DELETE rieid"
                    sqlcom = sqlcom + " FROM [received_import_expedition_invoice_detail] rieid INNER JOIN [received_import_expedition_invoice] riei ON rieid.[id_invoice] = riei.[id]"
                    sqlcom = sqlcom + " WHERE riei.[id] = " & CType(Me.dg_data.Items(x).FindControl("lbl_no_penerimaan"), LinkButton).Text
            ...
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    sqlcom = Nothing
                    sqlcom = "delete received_import_expedition_invoice"
                    sqlcom = sqlcom + " where id = " & CType(Me.dg_data.Items(x).FindControl("lbl_no_penerimaan"), LinkButton).Text
                    connection.koneksi.DeleteRecord(sqlcom)
            ...
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    Dim keterangan As String = "Penerimaan Invoice Ekspedisi no. " & CType(Me.dg_data.Items(x).FindControl("lbl_no_penerimaan"), LinkButton).Text
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Then
            Response.Redirect("~/detil_received_expedition_invoice.aspx?vid_received=" & CType(e.Item.FindControl("lbl_no_penerimaan"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text)
        End If
    End Sub
