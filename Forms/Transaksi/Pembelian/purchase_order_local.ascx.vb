...
Partial Class Forms_Transaksi_Pembelian_purchase_order_local
    ...
    Sub checkdata()
        ...
        sqlcom = sqlcom + " where purchase_order_local.id_transaction_period = " & Me.dd_bulan.SelectedValue
        reader = connection.koneksi.SelectRecord(sqlcom)
        reader.Read()
        If reader.HasRows Then
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = "select purchase_order_local.no, purchase_order_local.po_no_text,purchase_order_local.inv_no, convert(char, purchase_order_local.tanggal, 103) as tanggal"
            sqlcom = sqlcom + " from purchase_order_local"
            sqlcom = sqlcom + " where purchase_order_local.id_transaction_period = " & Me.dd_bulan.SelectedValue
            ...
            If Me.dd_pilihan.SelectedValue = "0" Then
                If String.IsNullOrEmpty(Me.tb_search.Text) Then
                    sqlcom = sqlcom
                Else
                    sqlcom = sqlcom + " and upper(purchase_order_local.po_no_text) like upper('%" & Me.tb_search.Text & "%')"
                End If
            ElseIf Me.dd_pilihan.SelectedValue = "1" Then
                If String.IsNullOrEmpty(Me.tb_search.Text) Then
                    sqlcom = sqlcom
                Else
                    sqlcom = sqlcom + " and upper(purchase_order_local.inv_no) like upper('%" & Me.tb_search.Text & "%')"
                End If
            End If
            sqlcom = sqlcom + " order by purchase_order_local.no"
            ...
            Using con
                con.Open()
                da.Fill(ds, "purchase_order_local")
                Me.dg_data.DataSource = ds.Tables("purchase_order_local").DefaultView

                If ds.Tables("purchase_order_local").Rows.Count > 0 Then
                    If ds.Tables("purchase_order_local").Rows.Count > 10 Then
    ...
    Protected Sub btn_new_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_new.Click
        Response.Redirect("~/detil_purchase_order_local.aspx?vtahun=" & Me.tb_tahun.Text & "&vbulan=" & Me.bulan)
    End Sub
        ...
        Try
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    sqlcom = "delete purchase_order_local"
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Then
            Response.Redirect("~/detil_purchase_order_local.aspx?vno_po=" & CType(e.Item.FindControl("lbl_no"), Label).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&vpaging=" & Me.dg_data.CurrentPageIndex)
        End If
    End Sub
