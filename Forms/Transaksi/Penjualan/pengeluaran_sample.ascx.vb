...
Partial Class Forms_Transaksi_Penjualan_pengeluaran_sample
    Inherits System.Web.UI.UserControl

    Public tradingClass As tradingClass = New tradingClass()

    Private Property id_sales() As Integer
        Get
            Dim o As Object = ViewState("id_sales")
            If Not o Is Nothing Then Return CInt(o) Else Return 0
        End Get
        Set(ByVal value As Integer)
            ViewState("id_sales") = value
        End Set
    End Property
    ...
    Sub bindcustomer()
        Try
            ...
            sqlcom = sqlcom + " where id = " & Me.tb_id_customer.Text
            reader = connection.koneksi.SelectRecord(sqlcom)
            reader.Read()
    ...
    Sub bindsales()
        Try
            ...
            sqlcom = "select code, nama_pegawai from user_list"
            sqlcom = sqlcom + " where code = (select code_sales from daftar_customer where id = " & Me.tb_id_customer.Text & ")"
            readersales = connection.koneksi.SelectRecord(sqlcom)
            readersales.Read()
            If readersales.HasRows Then
                Me.id_sales = readersales.Item("code").ToString
                Me.lbl_nama_sales.Text = readersales.Item("nama_pegawai").ToString
            End If
            readersales.Close()
            connection.koneksi.CloseKoneksi()
        Catch ex As Exception
    ...
    Sub checkdata()
        Try
            sqlcom = "select  '*'"
            sqlcom = sqlcom + " from pengeluaran_sample"
            sqlcom = sqlcom + " inner join sample on sample.id = pengeluaran_sample.id_sample"
            sqlcom = sqlcom + " inner join measurement_unit on measurement_unit.id = sample.id_satuan"
            sqlcom = sqlcom + " inner join daftar_customer on daftar_customer.id = pengeluaran_sample.id_customer"
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = "select  pengeluaran_sample.id, convert(char, pengeluaran_sample.tanggal, 103) as tanggal, pengeluaran_sample.id_customer,"
            sqlcom = sqlcom + " isnull(pengeluaran_sample.qty,0) as qty, pengeluaran_sample.id_transaction_period, pengeluaran_sample.id_sample,"
            sqlcom = sqlcom + " pengeluaran_sample.id_currency, isnull(pengeluaran_sample.harga,0) as harga, isnull(pengeluaran_sample.kurs,0) as kurs,"
            sqlcom = sqlcom + " pengeluaran_sample.id_sales, sample.name as nama_sample, measurement_unit.name as nama_satuan, daftar_customer.name as nama_customer,"
            sqlcom = sqlcom + " user_list.nama_pegawai"
            sqlcom = sqlcom + " from pengeluaran_sample"
            sqlcom = sqlcom + " inner join user_list on user_list.code = pengeluaran_sample.id_sales"
            sqlcom = sqlcom + " inner join sample on sample.id = pengeluaran_sample.id_sample"
            sqlcom = sqlcom + " inner join measurement_unit on measurement_unit.id = sample.id_satuan"
            sqlcom = sqlcom + " inner join daftar_customer on daftar_customer.id = pengeluaran_sample.id_customer"
            sqlcom = sqlcom + " where id_transaction_period = " & Me.dd_bulan.SelectedValue
            ...
            sqlcom = sqlcom + " order by pengeluaran_sample.tanggal"
            ...
            Using con
                con.Open()
                da.Fill(ds, "pengeluaran_sample")
                Me.dg_data.DataSource = ds.Tables("pengeluaran_sample").DefaultView

                If ds.Tables("pengeluaran_sample").Rows.Count > 0 Then
                    If ds.Tables("pengeluaran_sample").Rows.Count > 8 Then
    ...
    Protected Sub btn_save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_save.Click
        Try
            ...
            Else
                ...
                sqlcom = "select isnull(max(id),0) + 1 as vmax from pengeluaran_sample"
                reader = connection.koneksi.SelectRecord(sqlcom)
                reader.Read()
                If reader.HasRows Then
                ...
                sqlcom = "insert into pengeluaran_sample (id, tanggal, id_customer, id_sales, qty, id_transaction_period, id_sample, id_currency, harga, kurs)"
                sqlcom = sqlcom + " values(" & vmax & ",'" & vtgl & "'," & Me.tb_id_customer.Text & "," & Me.id_sales & "," & Decimal.ToDouble(Me.tb_qty.Text)
        ...
        Try
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    sqlcom = "delete pengeluaran_sample"
        ...
        Try
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    sqlcom = "update pengeluaran_sample"
