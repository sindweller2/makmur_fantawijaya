...
Partial Class Forms_Transaksi_Keuangan_hutang_bank
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = "select seq, convert(char, tanggal_terima, 103) as tgl_terima, isnull(jumlah,0) as jumlah, is_submit,"
            sqlcom = sqlcom + " keterangan, bank_account.id_currency as mata_uang,"
            sqlcom = sqlcom + " case"
            ...
            sqlcom = sqlcom + " from hutang_bank"
            sqlcom = sqlcom + " inner join bank_account on bank_account.id = hutang_bank.id_bank_account"
            ...
            Using con
                con.Open()
                da.Fill(ds, "hutang_bank")
                Me.dg_data.DataSource = ds.Tables("hutang_bank").DefaultView

                If ds.Tables("hutang_bank").Rows.Count > 0 Then
                    If ds.Tables("hutang_bank").Rows.Count > 10 Then
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Then
            If Me.dd_pilihan.SelectedValue = 0 Then
                Response.Redirect("~/hutang_bank_detil.aspx?vno_transaksi=" & CType(e.Item.FindControl("lbl_seq"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&voption=" & Me.dd_pilihan.SelectedValue & "&vsearch=" & Me.tb_search.Text)
            ElseIf Me.dd_pilihan.SelectedValue = 1 Then
                Response.Redirect("~/hutang_bank_detil.aspx?vno_transaksi=" & CType(e.Item.FindControl("lbl_seq"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&voption=" & Me.dd_pilihan.SelectedValue & "&vsearch=" & Me.dd_submit.SelectedValue)
            End If
        End If
    End Sub
    ...
    Protected Sub btn_new_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_new.Click
        Response.Redirect("~/hutang_bank_detil.aspx?vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text)
    End Sub
