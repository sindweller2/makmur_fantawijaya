...
Partial Class Forms_Transaksi_Collection_transfer_antar_kas_fin
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = "select seq, convert(char, tanggal_transfer, 103) as tanggal_transfer, isnull(nilai,0) as nilai, keterangan, is_submit, no_voucher,"
            sqlcom = sqlcom + " case"
            ...
            ElseIf Me.dd_pilihan.SelectedValue = "2" Then
                If String.IsNullOrEmpty(Me.tb_search.Text) Then
                    sqlcom = sqlcom
                Else
                    sqlcom = sqlcom + " and no_voucher like '%" & Me.tb_search.Text & "%'"
                End If
            End If
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Then
            If Me.dd_pilihan.SelectedValue <> 1 Then
                Response.Redirect("~/transfer_antar_kas_detil_fin.aspx?vid=" & CType(e.Item.FindControl("lbl_seq"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&voption=" & Me.dd_pilihan.SelectedValue & "&vsearch=" & Me.tb_search.Text & "&vpaging=" & Me.dg_data.CurrentPageIndex)
            ElseIf Me.dd_pilihan.SelectedValue = 1 Then
                Response.Redirect("~/transfer_antar_kas_detil_fin.aspx?vid=" & CType(e.Item.FindControl("lbl_seq"), LinkButton).Text & "&vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text & "&voption=" & Me.dd_pilihan.SelectedValue & "&vsearch=" & Me.dd_submit.SelectedValue & "&vpaging=" & Me.dg_data.CurrentPageIndex)
            End If
        End If
    End Sub
    ...
    Protected Sub btn_new_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_new.Click
        Response.Redirect("~/transfer_antar_kas_detil_fin.aspx?vbulan=" & Me.bulan & "&vtahun=" & Me.tb_tahun.Text)
    End Sub
