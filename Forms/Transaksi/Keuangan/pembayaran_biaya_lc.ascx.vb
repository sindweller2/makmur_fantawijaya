...
Partial Class Forms_Transaksi_Keuangan_pembayaran_biaya_lc
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = sqlcom + " convert(char, lc.tgl_bayar_biaya_lc, 103) as tgl_bayar_biaya_lc, lc.is_submit_bayar,"
            sqlcom = sqlcom + " case"
            sqlcom = sqlcom + " when lc.is_submit_bayar = 'B' then 'Belum disubmit'"
            sqlcom = sqlcom + " when lc.is_submit_bayar = 'S' then 'Sudah disubmit'"
            ...
            ElseIf Me.dd_pilihan.SelectedValue = "2" Then
                If Me.dd_sts_bayar.SelectedValue = "B" Then
                    sqlcom = sqlcom + " and lc.tgl_bayar_biaya_lc is null"
                Else
                    sqlcom = sqlcom + " and lc.tgl_bayar_biaya_lc is not null"
                End If
            ElseIf Me.dd_pilihan.SelectedValue = "3" Then
                If Me.dd_sts_submit.SelectedValue = "B" Then
                    sqlcom = sqlcom + " and lc.is_submit_bayar = 'B'"
                Else
                    sqlcom = sqlcom + " and lc.is_submit_bayar = 'S'"
                End If
            End If
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Or e.CommandName = "LinkLC" Then
            Response.Redirect("~/detil_pembayaran_biaya_lc.aspx?vno_lc=" & CType(e.Item.FindControl("lbl_seq_lc"), Label).Text & "&vtahun=" & Me.tb_tahun.Text & "&vbulan=" & Me.bulan)
        End If
    End Sub
