...
Partial Class Forms_Transaksi_Keuangan_pembayaran_pib
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = sqlcom + " case"
            sqlcom = sqlcom + " when entry_dokumen_impor.is_submit_pib = 'B' then 'Belum disubmit'"
            sqlcom = sqlcom + " when entry_dokumen_impor.is_submit_pib = 'S' then 'Sudah disubmit'"
            ...
            ElseIf Me.dd_pilihan.SelectedValue = "3" Then
                If Me.dd_sts_bayar.SelectedValue = "B" Then
                    sqlcom = sqlcom + " and upper(entry_dokumen_impor.tanggal_bayar_pib) is null"
                Else
                    sqlcom = sqlcom + " and upper(entry_dokumen_impor.tanggal_bayar_pib) is not null"
                End If
            ElseIf Me.dd_pilihan.SelectedValue = "4" Then
                If Me.dd_sts_submit.SelectedValue = "B" Then
                    sqlcom = sqlcom + " and upper(entry_dokumen_impor.is_submit_pib) = 'B'"
                Else
                    sqlcom = sqlcom + " and upper(entry_dokumen_impor.is_submit_pib) = 'S'"
                End If
            End If
    ...
    Protected Sub dg_data_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dg_data.ItemCommand
        If e.CommandName = "LinkItem" Or e.CommandName = "LinkDokumen" Then
            Response.Redirect("~/detil_pembayaran_pib.aspx?vno_dokumen=" & CType(e.Item.FindControl("lbl_no_dokumen"), LinkButton).Text & "&vtahun=" & Me.tb_tahun.Text & "&vbulan=" & Me.bulan)
        End If
    End Sub
    ...
    Sub pilihan()
        If Me.dd_pilihan.SelectedValue = "0" Or Me.dd_pilihan.SelectedValue = "1" Or Me.dd_pilihan.SelectedValue = "2" Then
            ...
            Me.dd_sts_bayar.Visible = False
            Me.dd_sts_submit.Visible = False
        ElseIf Me.dd_pilihan.SelectedValue = "3" Then
            Me.tb_search.Visible = False
            Me.btn_search.Visible = False
            Me.dd_sts_bayar.Visible = True
            Me.dd_sts_submit.Visible = False
        ElseIf Me.dd_pilihan.SelectedValue = "4" Then
            Me.tb_search.Visible = False
            Me.btn_search.Visible = False
            Me.dd_sts_bayar.Visible = False
            Me.dd_sts_submit.Visible = True
        End If
    End Sub
    ...
    Protected Sub dd_sts_bayar_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dd_sts_bayar.SelectedIndexChanged
        Me.pilihan()
        Me.loadgrid()
    End Sub

    Protected Sub dd_sts_submit_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dd_sts_submit.SelectedIndexChanged
        Me.pilihan()
        Me.loadgrid()
    End Sub
End Class
