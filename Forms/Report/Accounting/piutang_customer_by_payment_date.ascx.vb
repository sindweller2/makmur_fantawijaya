...
Partial Class Forms_Report_Accounting_piutang_customer_by_payment_date
    ...
    '        sqlcom = "delete temp_piutang_customer_by_payment_date"
    ...
    '        sqlcom = "insert into temp_piutang_customer_by_payment_date(id_sales, id_customer, no_so_text, nama_customer,"
    '        sqlcom = sqlcom + "nama_sales, tgl_invoice, tgl_jatuh_tempo, nilai_idr, nilai_usd, kurs)"
    ...
    Sub hapus_piutang()
        sqlcom = Nothing
        sqlcom = "delete from temp_piutang_customer_by_payment_date"
        connection.koneksi.DeleteRecord(sqlcom)
    End Sub
    ...
    Sub hitung_piutang_outstanding()
        Try
            ...
            sqlcom = "insert into temp_piutang_customer_by_payment_date(no_so_text, id_sales, id_customer, nama_customer,"
    ...
    Sub hapus_duplikat_data()
        Try
            sqlcom = Nothing
            sqlcom = sqlcom + " WITH CTE([no_so_text],[nama_customer],[nama_sales],[tgl_invoice],[tgl_jatuh_tempo],[nilai_idr],[nilai_usd],[id_customer],[id_sales],[kurs],[nilai_saldo_awal],[nilai_invoice],[nilai_bayar],[nilai_retur], duplicatecount)"
            sqlcom = sqlcom + " AS (SELECT [no_so_text],[nama_customer],[nama_sales],[tgl_invoice],[tgl_jatuh_tempo],[nilai_idr],[nilai_usd],[id_customer],[id_sales],[kurs],[nilai_saldo_awal],[nilai_invoice],[nilai_bayar],[nilai_retur], ROW_NUMBER() OVER(PARTITION BY [no_so_text],[nama_customer],[nama_sales],[tgl_invoice],[tgl_jatuh_tempo],[nilai_idr],[nilai_usd],[id_customer],[id_sales],[kurs],[nilai_saldo_awal],[nilai_invoice],[nilai_bayar],[nilai_retur] ORDER BY [no_so_text]) AS DuplicateCount FROM temp_piutang_customer_by_payment_date)"
        ...
        Try
            If Me.RadioButtonPDF.Checked = True And Me.RadioButtonExcel.Checked = False Then
                ...
                If Me.RadioButtonDetail.Checked = True And Me.RadioButtonSummary.Checked = False Then
                    'Dim reportPath As String = Server.MapPath("reports\penerimaan_pembayaran_penjualan_piutang_by_payment_date.rpt")
                    reportPath = Server.MapPath("reports\lap_piutang_customer_by_payment_date.rpt")
                    file_pdf = "Pdf_files/lap_piutang_customer_by_payment_date.pdf"
                ElseIf Me.RadioButtonDetail.Checked = False And Me.RadioButtonSummary.Checked = True Then
                    reportPath = Server.MapPath("reports\lap_piutang_customer_by_payment_date_summary.rpt")
                    file_pdf = "Pdf_files/lap_piutang_customer_by_payment_date_summary.pdf"
                End If
            ...
            ElseIf Me.RadioButtonPDF.Checked = False And Me.RadioButtonExcel.Checked = True Then
                ...
                If Me.RadioButtonDetail.Checked = True And Me.RadioButtonSummary.Checked = False Then
                    ...
                    sqlcom += " from temp_piutang_customer_by_payment_date"
                ...
                ElseIf Me.RadioButtonDetail.Checked = False And Me.RadioButtonSummary.Checked = True Then
                    ...
                    sqlcom = "select nama_customer as 'Nama Customer',sum(isnull(nilai_saldo_awal,0)) as 'Saldo Awal',sum(isnull(nilai_invoice,0)) as 'Penjualan',sum(isnull(nilai_bayar,0)) as 'Pembayaran',sum(isnull(nilai_retur,0)) as 'Retur',sum(isnull(nilai_idr,0)) as 'Nilai IDR',sum(isnull(nilai_usd,0)) as 'Nilai USD'"
                    sqlcom += " from temp_piutang_customer_by_payment_date"
