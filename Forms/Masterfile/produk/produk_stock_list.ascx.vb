...
Partial Class Forms_Masterfile_produk_produk_stock_list
    ...
    Sub update_stock_masuk()
        Try
            sqlcom = "select product_stock.id_product, isnull((select isnull(sum(isnull(qty_terima,0)),0)"
            sqlcom = sqlcom + " from entry_dokumen_impor_produk"
            sqlcom = sqlcom + " where entry_dokumen_impor_produk.seq_entry in"
            sqlcom = sqlcom + " (select seq from entry_dokumen_impor"
            sqlcom = sqlcom + " where entry_dokumen_impor.is_submit_gudang = 'S'"
            sqlcom = sqlcom + " and entry_dokumen_impor.id_transaksi_terima_barang = " & Me.dd_bulan.SelectedValue & ")"
            sqlcom = sqlcom + " and entry_dokumen_impor_produk.id_product = product_stock.id_product"
            sqlcom = sqlcom + " group by entry_dokumen_impor_produk.id_product),0) as stock_masuk"
