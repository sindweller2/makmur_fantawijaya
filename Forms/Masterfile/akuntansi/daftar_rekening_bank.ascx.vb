...
Partial Class Forms_Masterfile_akuntansi_daftar_rekening_bank
    ...
    Sub bindakun()
        ...
        sqlcom = sqlcom + " where substring(rtrim(accountno),1,5) = '11.02'"
        sqlcom = sqlcom + " and convert(int, right(rtrim(accountno),2)) < 11"
    ...
    Sub loadgrid()
        Try
            ...
            Me.tb_name.Text = ""
            Me.tb_atas_nama.Text = ""

            connection.koneksi.Openkoneksi()
            Dim con As New SqlClient.SqlConnection(connection.koneksi.con.ConnectionString)

            sqlcom = "select id, name, id_bank, account_code, id_currency, atas_nama, isnull(saldo_akhir,0) as saldo_akhir"
            ...
            Using con
                con.Open()
                da.Fill(ds, "bank_account")
                Me.dg_data.DataSource = ds.Tables("bank_account").DefaultView

                If ds.Tables("bank_account").Rows.Count > 0 Then
                    If ds.Tables("bank_account").Rows.Count > 10 Then
                    ...
                    For x As Integer = 0 To Me.dg_data.Items.Count - 1
                        ...
                        CType(Me.dg_data.Items(x).FindControl("dd_bank"), DropDownList).SelectedValue = CType(Me.dg_data.Items(x).FindControl("lbl_id_bank"), Label).Text
                        reader.Close()
                        connection.koneksi.CloseKoneksi()
    ...
    Protected Sub btn_save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_save.Click
        Try
            If String.IsNullOrEmpty(Me.tb_name.Text) Then
                Me.lbl_msg.Text = "Silahkan mengisi nomor rekening terlebih dahulu"
            ElseIf String.IsNullOrEmpty(Me.tb_atas_nama.Text) Then
                Me.lbl_msg.Text = "Silahkan mengisi atas nama terlebih dahulu"
            Else
                ...
                sqlcom = "insert into bank_account(id, name, id_bank, account_code, id_currency, atas_nama, is_bank)"
                sqlcom = sqlcom + " values(" & vmax & ",'" & Me.tb_name.Text & "'," & Me.dd_bank.SelectedValue & ",'" & Me.dd_akun.SelectedValue & "',"
                sqlcom = sqlcom + "'" & Me.dd_mata_uang.SelectedValue & "','" & Me.tb_atas_nama.Text & "','Y')"
        ...
        Try
            For x As Integer = 0 To Me.dg_data.Items.Count - 1
                If CType(Me.dg_data.Items(x).FindControl("cb_data"), CheckBox).Checked = True Then
                    ...
                    sqlcom = sqlcom + " set name = '" & CType(Me.dg_data.Items(x).FindControl("tb_rekening"), TextBox).Text & "',"
                    ...
                    sqlcom = sqlcom + " atas_nama = '" & CType(Me.dg_data.Items(x).FindControl("tb_atas_nama"), TextBox).Text & "'"
                    sqlcom = sqlcom + " where id = " & CType(Me.dg_data.Items(x).FindControl("lbl_id"), Label).Text
