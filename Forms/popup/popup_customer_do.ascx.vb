...
Partial Class Forms_Popup_popup_customer_do
    ...
    Sub loadgrid()
        Try
            ...
            sqlcom = "select daftar_customer.id, daftar_customer.name"
            sqlcom = sqlcom + " from daftar_customer"
            sqlcom = sqlcom + " inner join sales_order on sales_order.id_customer = daftar_customer.id"
            sqlcom = sqlcom + " where daftar_customer.status = '1'"

            If String.IsNullOrEmpty(Me.tb_search.Text) Then
                sqlcom = sqlcom
            Else
                sqlcom = sqlcom + " and upper(daftar_customer.name) like upper('%" & Me.tb_search.Text & "%')"
            End If
